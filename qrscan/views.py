...
def Ticket_display(request):
    ...
    customers = MonumentTicket.objects.filter(date=d3,paid=True)
...
def display(request,monuments):
    ...
    customers = MonumentTicket.objects.filter(monument=monuments)
    myfilter=DetailFilter(request.GET,queryset=customers)
    customers=myfilter.qs
    customer = MonumentTicket.objects.filter(monument=monuments)
    if customer.count()!=0:
        df=customer.values()
        df=pd.DataFrame(df)
        ...
        l=[a[True],a[False]]
        chart=get_chart(l)
        return render(request,'detail.html',{"object_list":customers,"filter":myfilter,"chart":chart})
...
def Admin(request):
    ...
    today=date.today()
    d3 = today.strftime("%Y-%m-%d")
    customers = MonumentTicket.objects.filter(date=d3,verified=True)
    for id in customers:
...
def Counter(request):
    ...
    for m in monuments:
        count=0
        mo= MonumentTicket.objects.filter(monument=m,date=d3,verified=True)
        for id in mo:
            count=count+id.total_count
        counter[m]=count
    print(counter)
